...
pred = Predictor(input_dim=20, hidden_dim=16, output_dim=20); \
...
target_features = torch.randn(8, 20); \
...
out_path = os.path.join('results', 'profile.txt'); \
os.makedirs('results', exist_ok=True); \
open(out_path, 'w').write(s.getvalue()); \
